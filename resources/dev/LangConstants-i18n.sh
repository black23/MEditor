...
cp $APPDIR"$PATH_TO_CONSTANTS"/"$NAME"_en.properties $APPDIR"$PATH_TO_CONSTANTS"/"$NAME".properties
mkdir -p $APPDIR/src/cz/mzk/editor/client
cp $APPDIR"$PATH_TO_CONSTANTS"/*.properties $APPDIR/src/cz/mzk/editor/client/

java -cp "$APPDIR/src:$APPDIR/bin:$GWT_HOME/gwt-user.jar:$GWT_HOME/gwt-dev.jar" com.google.gwt.i18n.tools.I18NSync -out $APPDIR"$PATH_TO_JAVA_CLASS"  cz.mzk.editor.client."$NAME" ;
